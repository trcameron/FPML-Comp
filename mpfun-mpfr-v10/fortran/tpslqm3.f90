...
!   David H Bailey   22 May 2020

!  COPYRIGHT AND DISCLAIMER:
!    All software in this package (c) 2020 David H. Bailey.
...
!   These parameters are set in parameter statements below:
...
!     n     Integer relation vector length. When kq = 0, n = 1 + polynomial
!             degree; default for this program (when kq = 0) = kr * ks + 1 = 73.
!     ndp   Full precision level in digits; default for this program = 1100.
!           ***Must be <= mpipl in module MPFUNF.
!           ***Must be >= ndpm + precision level required to find relation.
!     ndpm  Medium precision level in digits, typically ndp / 10 or so;
!             default for this program = 120.
...
!     nrb   Log10 of maxinum size (Euclidean norm) of acceptable relation;
...
integer itr, kq, kr, ks, lcx
parameter (itr = 1, kq = 0, kr = 8, ks = 9, lcx = 64)
integer idb, n, ndp, ndpm, ndr, nep, nepm, nrb, nwds, nwdsm
parameter (idb = 2, n = kr * ks + 1, ndp = 1100, ndpm = 120, ndr = 20, &
  nep = 30 - ndp, nepm = 20 - ndpm, nrb = 200, nwds = int (ndp / mpdpw + 2), &
  nwdsm = int (ndpm / mpdpw + 2))

real (8) d1, d2, second, tm, tm0, tm1
...
write (6, 1) itr, n, kq, kr, ks, ndr, nrb, ndp, ndpm, nep, nepm
1 format ('PSLQM3 Test Program'/ &
  'No. trials = ',i3,3x,'n =',i4,3x,'kq =',i2/ &
  'kr =',i2,3x,'ks =',i2,3x,'ndr =',i5,3x,'nrb =',i5/ &
...
!   Perform relation search.
  ...
  tm0 = second ()
  call pslqm3 (idb, n, nwds, nwdsm, ndr, nrb, eps, epsm, x, iq, r)
  tm1 = second ()
...
subroutine pslqm3 (idb, n, nwds, nwdsm, ndr, nrb, eps, epsm, x, iq, r)
...
!     nwdsm int*4   Medium precision level in words, typically nwds / 10 or so.
!     ndr   int*4   Log10 of the min dynamic range at detection, typically 20
!                     or 30. A detected relation is not deemed reliable unless
!                     this is exceeded.
!     nrb   int*4   Log10 of max size (Euclidean norm) of acceptable relation,
...
!     epsm  mp_real Tolerance for medium precision detection. This is
...
!     itm   int*4   Maximum iteration count; default = 10^7. Run is aborted if
!                     this is exceeded. If itm >= 10^8, change all "i8" in
!                     formats in the subroutines below to "i9" or as needed.
!     ndrm  int*4   Extra precision beyond dynamic range for MPM iterations;
!                     default = 50.
...
!                   When nrs = 2, input restart file is overwritten, so make
!                   a backup copy before running this program.
...
integer i, idb, imq, ipi, ipm, iq, it, itm, its, izd, izm, izmm, j, j1, n, &
  n1, n2, n3, n4, ndr, ndrm, nrb, nrs, nsq, nwds, nwdsm, ndpm2, nwdsm2
parameter (ipi = 500, ipm = 10, itm = 10000000, ndrm = 50, nrs = 0, nsq = 8)
real (8) d1, d2, d3, d4, deps, dplog10, rb, second, &
  tm0, tm1, times(6)
...
type (mp_real) bound, dynrange, epsm, epsm2, wa(n,n), &
...
!   Check the dynamic range of y vector.
...
5 format ('Iteration',i8,2x,'MPM initialization')
tm0 = second ()
...
w1 = dynrange (n, nwdsm, wy)
if (idb >= 3) then
  call decmd (w1, d1, n1)
  write (6, 6) it, d1, n1
6 format ('Iteration',i8,2x,'Min/max ratio in wy =',f11.6,'e',i6)
endif
...
7 format ('Iteration',i8,2x,'Start DP iterations')
...
9 format ('Iteration',i8,2x,'MPM update')
  tm0 = second ()
...
10  format ('Iteration',i8,2x,'MP update')
    tm0 = second ()
...
11    format ('Iteration',i8,2x,'Norm bound =',f11.6,'e',i5,3x, &
...
!   MPM iterations. Perform these with a working precision level that is at
!   least ndrm digits greater than the dynamic range of the wy vector.

w1 = dynrange (n, nwdsm, wy)
call decmd (w1, d1, n1)
ndpm2 = ndrm - n1
nwdsm2 = int (ndpm2 / mpdpw) + 2
epsm2 = mpreal (2.d0, nwdsm2) ** (70 - nwdsm2 * mpnbt)

if (idb >= 2) write (6, 14) it, ndpm2
14 format ('Iteration',i8,2x,'Start MPM iterations: precision =',i6,' digits')

!   Perform LQ decomposition using MPM precision.

tm0 = second ()
call lqmp (n, n - 1, nwdsm2, wh)
tm1 = second ()
times(2) = times(2) + (tm1 - tm0)
...
!   at least IPM iterations in MPM).
  ...
  if (mod (it - its, ipm) == 0) then
    w1 = dynrange (n, nwdsm2, wy)
    call decmd (w1, d1, n1)
    if (idb >= 2) then
      write (6, 16) it, d1, n1
16    format ('Iteration',i8,2x,'Min/max ratio in wy =',f11.6,'e',i6)
    endif
    if (w1 > mpreald (deps, nwdsm2)) then 
      if (idb >= 2) write (6, 17) it
17    format ('Iteration',i8,2x,'Return to DP iterations')
      goto 110
    endif

!   Change the working precision level periodically to be at least ndrm digits
!   greater than the dynamic range of the wy vector.

    ndpm2 = ndrm - n1
    nwdsm2 = int (ndpm2 / mpdpw) + 2
    epsm2 = mpreal (2.d0, nwdsm2) ** (70 - nwdsm2 * mpnbt)
    write (6, 31) it, ndpm2
31  format ('Iteration',i8,2x,'Resume MPM iterations: precision =',i6,' digits')
  endif
  goto 140
elseif (izmm == 1) then
...
18 format ('Iteration',i8,2x,'MP update')
  tm0 = second ()
  call updtmp (idb, it, n, nwds, wa, wb, eps, b, h, y, izm)
  tm1 = second ()
  times(5) = times(5) + (tm1 - tm0)
...
19 format ('Iteration',i8,2x,'Relation detected'/ &
...
if (n3 <= nrb .and. n2 - n1 >= ndr) then
  ...
  if (idb >= 2) write (6, 21)
21 format ('Relation is too large or insufficient dynamic range in y at detection.')
endif
...
!   normally done using MPM precision.
...
3 format ('initmpm: wy vector:')
  call matoutmd (1, n, ix, dx, wy)
  write (6, 4)
4 format ('initmpm: Factored wh matrix:')
  call matoutmd (n, n - 1, ix, dx, wh)
endif
...
5 format ('itermpm: Updated wy:')
  call matoutmd (1, n, ip, dx, wy)
  write (6, 6)
6 format ('itermpm: Updated wa matrix:')
  call matoutmd (n, n, ip, dx, wa)
  write (6, 7)
7 format ('itermpm: Updated wb matrix:')
  call matoutmd (n, n, ip, dx, wb)
  write (6, 8)
8 format ('itermpm: Updated wh matrix:')
  call matoutmd (n, n - 1, ip, dx, wh)
endif
...
subroutine lqmp (n, m, nwdsm, h)

!   This performs an LQ decomposition on the MPM matrix h.  It is a simplified
!   and transposed adaptation of the subroutine dqrdc from Linpack.
!   This is normally performed in medium precision.
!   Input: n, m, nwdsm, h.
!   Output: h.
...
6 format ('updtmpm: Updated wa matrix:')
  call matoutmd (n, n, ix, dx, wa)
  write (6, 7)
7 format ('updtmpm: Updated wb matrix:')
  call matoutmd (n, n, ix, dx, wb)
  write (6, 8)
...
!   For input MP or MPM value a, this routine returns a DP approximation to log10 (a).
...
!   For input MP or MPM value a, this routine returns DP b and integer ib such that 
...
!   This outputs the MP or MPM matrix a as a DP matrix.
...
!   This multiplies the MP or MPM square matrix a by the MP matrix b, and the
...
!   This routine sorts the entries of the N-long MP or MPM vector A into ascending
!   order using the quicksort algorithm.  The permutation vector that would
!   sort the vector is returned in IP.
!   This is performed in medium precision.
!   Input: n, a, ip.
