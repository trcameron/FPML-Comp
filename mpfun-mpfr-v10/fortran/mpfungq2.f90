...
!  Variant 2: Precision level specifications are *required*; includes real*16 support.
!  Search for !> for variant differences.

!  Revision date:  2 Mar 2020
...
private &
  ...
  mp_mdi, mp_min, mp_mod, mp_nrt, mp_outr, mp_outz, mp_pi, mp_polylog, &
...
!  Value is negative and exponent is negative or zero.
  ...
  function mp_gammainc (ra, rb)
    ...
    mpnwbt = max (ra%mpr(1), rb%mpr(1))
    mpnwbt = min (mpnwbt, mpwdsbt)
    call mp_initvr (mp_gammainc, mpnwbt)
    call mpfr_gamma_inc (mp_gammainc%mpr(1), ra%mpr(1), rb%mpr(1), %val(mprnd))
    return
  end function
...
!>  Otherwise in variant #2, uncomment these two lines:
  ...
  function mp_mod (ra, rb)
    ...
    call mpfr_fmod (mp_mod%mpr(1), ra%mpr(1), rb%mpr(1), %val(mprnd))
    return
  end function
...
!  If real*16 is enabled, uncomment the next two lines and comment the following two.
!  Otherwise comment the next two and uncomment the following two.
...
!  If real*16 is enabled, uncomment the next line and comment the following line.
!  Otherwise comment the next line and uncomment the following line.
...
!    real (kind (0.d0)):: mp_rtoq
    ...
    type (mp_real), intent (in):: ra
    type (mp_real) r1, r2
    real (mprknd) mpfr_get_d, d1, d2, d3
    external mpfr_get_d
    integer mpnwbt
    mpnwbt = min (ra%mpr(1), mpwdsbt)
    call mp_fixlocr (ra)
    call mp_initvr (r1, mpnwbt)
    call mp_initvr (r2, mpnwbt)
    d1 = mpfr_get_d (ra%mpr(1), %val(mprnd))
    mp_rtoq = d1
    call mpfrsubd (r1%mpr(1), ra%mpr(1), d1, mprnd)
    d2 = mpfr_get_d (r1%mpr(1), %val(mprnd))
    mp_rtoq = mp_rtoq + d2
    call mpfrsubd (r2%mpr(1), r1%mpr(1), d2, mprnd)
    d3 = mpfr_get_d (r2%mpr(1), %val(mprnd))
    mp_rtoq = mp_rtoq + d3
    return
  end function
