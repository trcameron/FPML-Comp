...
!             degree; default for this program (when kq = 0) = kr * ks + 1 = 57.
!     ndp   Full precision level in digits; default for this program = 650.
...
!             default = 100. Run will be aborted if this is exceeded.
...
parameter (itr = 1, kq = 0, kr = 7, ks = 8, lcx = 64)
integer idb, n, ndp, ndr, nep, nrb, nwds
parameter (idb = 2, n = kr * ks + 1, ndp = 650, ndr = 20, &
  nep = 20 - ndp, nrb = 100, nwds = int (ndp / mpdpw + 2))
...
!   Perform relation search.
  ...
  tm0 = second ()
  call pslqm2 (idb, n, nwds, ndr, nrb, eps, x, iq, r)
  tm1 = second ()
...
subroutine pslqm2 (idb, n, nwds, ndr, nrb, eps, x, iq, r)
...
real (8) bounddp, d1, d2, d3, d4, rn, second, tm0, tm1, &
  times(4)
